...
int parse_int(char *str, int *n, int i) {
    ...
    *n = strtol(str,&endptr, 10);
	...
	printf("argv[%d] is negative \n", i);
	return 0;
    } else if ((*n == 0) && (i != 3)) {
	printf("argv[%d] should be positive \n", i);
	return 0;
    } else
	return 1;
}
...
void *checker( void *a) {
    ...
    int delta = 0;		/* Maximum difference between threads */
    ...
    /* This is the last question: need to check differences among threads */

    if( arg->argc == 4 ) {
	if( parse_int(arg->argv[3], &delta, 3) == 0 ) {
	    printf("checker: exiting.\n");
	    pthread_exit(NULL);
	}
	delta *= 1000;	/* input arguments are in 1000s */
    }
	...
	/* Check delta */
	if( delta != 0 ) {
	    if( max(cnt[1][0],cnt[1][1]) - min(cnt[1][0],cnt[1][1]) > delta )
		printf("Checker: maximum difference is tooo large\n");
	}
