...
int main(int argc, char *argv[]) {
    ...
    for(int I = 0; I < NT; ++I) {
        ...
        sem_init(&Semaphores[I], 0, MaxDiff);
        Threads[I].Semaphores = Semaphores;
    ...
    // NOTE: Verification
    int Count[NT] = {0};
    for(int Idx = 0; Idx < vector.len; ++Idx) {
        ++Count[vector.array[Idx]];
        if(abs(Count[0] - Count[1]) > MaxDiff) {
            printf("Found a difference of %d :(\n", abs(Count[0] - Count[1]));
            exit(10);
        }
    }
