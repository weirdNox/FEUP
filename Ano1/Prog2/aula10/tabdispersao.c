...
const char* tabela_valor(tabela_dispersao *td, const char *chave)
{
    ...
    unsigned long hash = td->hfunc(chave, td->tamanho);
    elemento *el = td->elementos[hash];
    while(el && el->obj)
    {
        if(strcmp(el->obj->chave, chave) == 0)
        {
            return el->obj->valor;
        }
        el = el->proximo;
...
int tabela_alterahash(tabela_dispersao *h, hash_func *f)
{
    if(!h || !f)
    {
        return -1;
    ...
    h->hfunc = f;
    elemento **elementos = calloc(h->tamanho, sizeof(*elementos));
    for(int i = 0; i < h->tamanho; ++i)
    {
        elemento *el = h->elementos[i];
        while(el)
        {
            elemento *prox = el->proximo;

            unsigned long hash = f(el->obj->chave, h->tamanho);
            el->proximo = elementos[hash];
            elementos[hash] = el;
