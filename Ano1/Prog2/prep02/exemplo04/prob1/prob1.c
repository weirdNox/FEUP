...
pilha* fila_transforma(fila *f)
{
    ...
    pilha *resultado = pilha_nova();
    pilha *aux = pilha_nova();
    if(!resultado || !aux)
    {
        pilha_apaga(resultado);
        pilha_apaga(aux);
        return 0;
    }

    filaItem *item = f->cabeca;
    while(item)
    {
        pilha_push(aux, item->string);
        item = item->proximo;
    }

    while(aux->tamanho)
    {
        pilha_push(resultado, aux->raiz->elemento);
        pilha_pop(aux);
    }
...
int pilha_remove(pilha *p, const char* string)
{
    ...
    pilhaItem *prev = 0;
    pilhaItem *item = p->raiz;
    while(item)
    {
        if(strcmp(string, item->elemento) == 0)
        {
            if(!prev)
            {
                p->raiz = item->proximo;
...
tabela_dispersao tabela_copia_todos(tabela_dispersao origem, hash_func * h)
{
    ...
    {
        elemento *el = origem->elementos[i];
        while(el)
        {
            tabela_adiciona(td, el->obj);
            el = el->proximo;
        }
    }
