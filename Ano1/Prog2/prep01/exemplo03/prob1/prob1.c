...
int tira_carta(lista* baralho, lista* ordem)
{
    ...
    elemento *carta = baralho->inicio;
    int i; elemento *el;
    for(i = 0, el = ordem->inicio; el; el = el->proximo, ++i)
    {
        if(strcmp(carta->str, el->str) == 0)
        {
            break;
        }
    }

    lista_remove(baralho, carta);
...
char* faz_jogada(vetor *jogadores, lista *baralho, lista *ordem)
{
    ...
    {
        int carta = tira_carta(baralho, ordem);
        if(carta > melhor)
        {
            melhor = carta;
