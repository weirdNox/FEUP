...
vetor *vetor_concatena(vetor *v1, vetor *v2)
{
    vetor *result = vetor_novo();
    int v1Size = vetor_tamanho(v1), v2Size = vetor_tamanho(v2);
    for(int i = 0; i < v1Size; ++i)
    {
        vetor_insere(result, vetor_elemento(v1, i), i);
    }
    for(int i = 0; i < v2Size; ++i)
    {
        vetor_insere(result, vetor_elemento(v2, i), i+v1Size);
    }
...
{
    int vSize = vetor_tamanho(vec);
    for(int i = 0; i < vSize/2; ++i)
    {
        elemento el = vec->elementos[i];
        vec->elementos[i] = vec->elementos[vSize-1-i];
        vec->elementos[vSize-1-i] = el;
    }
}
...
vetor *vetor_baralha(vetor *vec)
{
    ...
    {
        vetor_insere(result, vetor_elemento(vec, i), i);
    }

    for(int i = 0; i < size-1; ++i)
    {
        int j = i + rand() % (size-i-1);
        elemento el = result->elementos[i];
        result->elementos[i] = result->elementos[j];
        result->elementos[j] = el;
    }
